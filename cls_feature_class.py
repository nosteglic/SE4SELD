...
class FeatureClass:
    def __init__(self, params_dict, is_eval=False):
        """
        ...
        # Input directories
        self._feat_label_dir = params_dict['feat_label_dir']
        self._dataset_dir = params_dict['dataset_dir']
        self._dataset_combination = '{}_{}'.format(params_dict['dataset'], 'eval' if is_eval else 'dev')
        ...
        self._fs = params_dict['fs']
        self._hop_len_s = params_dict['hop_len_s']
        self._hop_len = int(self._fs * self._hop_len_s)

        self._label_hop_len_s = params_dict['label_hop_len_s']
        ...
        self._nb_mel_bins = params_dict['nb_mel_bins']
        self._mel_wts = librosa.filters.mel(sr=self._fs, n_fft=self._nfft, n_mels=self._nb_mel_bins).T

        self._dataset = params_dict['dataset']
        self._eps = 1e-8
        self._nb_channels = 4

        # Sound event classes dictionary
        self._unique_classes = params_dict['unique_classes']
        self._audio_max_len_samples = params_dict['max_audio_len_s'] * self._fs  # TODO: Fix the audio synthesis code to always generate 60s of
    ...
    def _spectrogram(self, audio_input):
        ...
        for ch_cnt in range(_nb_ch):
            stft_ch = librosa.core.stft(np.asfortranarray(audio_input[:, ch_cnt]), n_fft=self._nfft,
                                        hop_length=self._hop_len, win_length=self._win_len, window='hann')
    ...
    def write_output_format_file(self, _output_format_file, _output_format_dict):
        ...
        for _frame_ind in _output_format_dict.keys():
            for _value in _output_format_dict[_frame_ind]:
                ...
                _fid.write('{},{},{},{},{},{}\n'.format(int(_frame_ind), int(_value[0]), 0,
                                                        float(_value[1]), float(_value[2]), float(_value[3])))
        _fid.close()
            ...
            if len(_tmp_ind[0]):
                ...
                for _tmp_class in _tmp_ind[0]:
                    if _is_polar:
                        ...
                        _output_dict[_frame_ind].append([_tmp_class, _x[_frame_ind, _tmp_class],
                                                         _y[_frame_ind, _tmp_class], _z[_frame_ind, _tmp_class]])
        return _output_dict
