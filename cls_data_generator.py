...
class DataGenerator(object):
    def __init__(
            self, params_dict, split=1, shuffle=True, per_file=False, is_eval=False
    ):
        self._per_file = per_file
        self._is_eval = is_eval
        self._splits = np.array(split)
        self._batch_size = params_dict['batch_size']
        self._feature_seq_len = params_dict['feature_sequence_length']
        self._label_seq_len = params_dict['label_sequence_length']
        self._shuffle = shuffle
        self._feat_cls = cls_feature_class.FeatureClass(params_dict=params_dict, is_eval=self._is_eval)
        self._label_dir = self._feat_cls.get_label_dir()
        self._feat_dir = self._feat_cls.get_normalized_feat_dir()

        self._filenames_list = list()
        self._nb_frames_file = 0  # Using a fixed number of frames in feat files.
                                  # Updated in _get_label_filenames_sizes()
