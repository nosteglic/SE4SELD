...
from keras.layers import (Bidirectional, Conv2D, MaxPooling2D, Concatenate,
                          Dense, Dropout, Permute, GlobalAveragePooling2D,
                          add, Activation, Input, Reshape, ELU, multiply)
...
def get_model(data_in, data_out, dropout_rate, nb_cnn2d_filt, f_pool_size, t_pool_size,
              rnn_size, fnn_size, weights, doa_objective, baseline, am='scSE', res='main', ratio=4):
    ...
    for i, convCnt in enumerate(f_pool_size):
        ...
        if baseline is False:
            spec_cnn = residual_squeeze_excite(input_tensor=spec_cnn, filters=nb_cnn2d_filt, res=res, ratio=ratio)
        else:
            spec_cnn = Conv2D(filters=nb_cnn2d_filt, kernel_size=(3, 3), padding='same')(spec_cnn)
            spec_cnn = BatchNormalization()(spec_cnn)
            spec_cnn = Activation('relu')(spec_cnn)
            if am == 'cSE':
                spec_cnn = squeeze_excite_block(spec_cnn)
            elif am == 'sSE':
                spec_cnn = spatial_squeeze_excite_block(spec_cnn)
            elif am == 'scSE':
                spec_cnn = channel_spatial_squeeze_excite(spec_cnn)
...
def masked_mse(y_gt, model_out):
    ...
    return keras.backend.sqrt(keras.backend.sum(keras.backend.square(y_gt[:, :, 14:] - model_out[:, :, 14:])
                                                * sed_out))/keras.backend.sum(sed_out)
...
def residual_squeeze_excite(input_tensor, filters, ratio=4, res='main', am='scSE'):
    spec_cnn = input_tensor
    spec_cnn = Conv2D(filters=filters, kernel_size=(3, 3), padding='same')(spec_cnn)
    spec_cnn = BatchNormalization()(spec_cnn)
    ...
    input_tensor = Conv2D(filters=filters, kernel_size=(1, 1), padding='same')(input_tensor)
    input_tensor = BatchNormalization()(input_tensor)
    if res == 'shortcut':
        if ratio != 0:
            if am == 'cSE':
                input_tensor = squeeze_excite_block(input_tensor)
            elif am == 'sSE':
                input_tensor = spatial_squeeze_excite_block(input_tensor)
            elif am == 'scSE':
                input_tensor = channel_spatial_squeeze_excite(input_tensor)

    spec_cnn = add([spec_cnn, input_tensor])
    spec_cnn = ELU()(spec_cnn)
