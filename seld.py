...
def collect_test_labels(_data_gen_test, _data_out, _nb_classes, quick_test):
    ...
    nb_batch = 2 if quick_test else _data_gen_test.get_total_batches_in_data()      # batch数量
    batch_size = _data_out[0][0]                                                    # batch大小
...
def save_result(data, params_dict):
    path = 'csv/{}.csv'.format(params_dict['model_name'])
...
'''训练'''
def train(params_dict, widget):
    # if widget_text:
    #     widget_text.tag_configure('stderr', foreground='#b22222')
    #     sys.stdout = textRedirector.TextRedirector(widget_text, 'stdout')
    #     sys.stderr = textRedirector.TextRedirector(widget_text, 'stderr')
    keras.backend.clear_session() # 多线程
    print('\n\nratio set by user input argument to: {}'.format(params_dict['ratio']))
    print('network baseline set by user input argument to: {}'.format(params_dict['do_baseline']))
    print('attention module choose: {}\n\n'.format(params_dict['am']))

    data = []
    data_format = '{}-{}'.format(params_dict['mode'], params_dict['dataset'])

    feat_cls = cls_feature_class.FeatureClass(params_dict)
    train_splits, val_splits, test_splits = None, None, None

    if params_dict['mode'] == 'dev':
        test_splits = [1]
        val_splits = [2]
        train_splits = [[3, 4, 5, 6]]

    elif params_dict['mode'] == 'eval':
        test_splits = [[7, 8]]
        val_splits = [[1]]
    ...
    for split_cnt, split in enumerate(test_splits):
        ...
        cls_feature_class.create_folder(params_dict['model_dir'])

        unique_name = '{}_{}_{}_split{}'.format(
            params_dict['model_name'], params_dict['dataset'], params_dict['mode'], split
        )
        unique_name = os.path.join(params_dict['model_dir'], unique_name)
        ...
        data_gen_train = cls_data_generator.DataGenerator(
            params_dict=params_dict, split=train_splits[split_cnt]
        )
        print('Loading validation dataset:')
        data_gen_val = cls_data_generator.DataGenerator(
            params_dict=params_dict, split=val_splits[split_cnt], shuffle=False
        )
        ...
        gt = collect_test_labels(data_gen_val, data_out, nb_classes, params_dict['quick_test'])
        sed_gt = evaluation_metrics.reshape_3Dto2D(gt[0])
        doa_gt = evaluation_metrics.reshape_3Dto2D(gt[1])

        print('MODEL:\n\tdropout_rate: {}\n\tCNN: nb_cnn_filt: {}, f_pool_size{}, t_pool_size{}'
              '\n\trnn_size: {}, fnn_size: {}\n\tdoa_objective: {}\n'.format(
               params_dict['dropout_rate'], params_dict['nb_cnn2d_filt'],
               params_dict['f_pool_size'], params_dict['t_pool_size'],
               params_dict['rnn_size'], params_dict['fnn_size'], params_dict['doa_objective']))

        print('Using loss weights : {}'.format(params_dict['loss_weights']))
        model = keras_model.get_model(data_in=data_in, data_out=data_out, dropout_rate=params_dict['dropout_rate'],
                                      nb_cnn2d_filt=params_dict['nb_cnn2d_filt'],
                                      f_pool_size=params_dict['f_pool_size'], t_pool_size=params_dict['t_pool_size'],
                                      rnn_size=params_dict['rnn_size'], fnn_size=params_dict['fnn_size'],
                                      weights=params_dict['loss_weights'], doa_objective=params_dict['doa_objective'],
                                      baseline=params_dict['do_baseline'], am=params_dict['am'],
                                      res=params_dict['res'], ratio=params_dict['ratio'])

        best_seld_metric = 99999
        best_epoch = -1
        patience_cnt = 0
        nb_epoch = 2 if params_dict['quick_test'] else params_dict['nb_epochs']
        ...
        for epoch_cnt in range(nb_epoch):
            ...
            hist = model.fit_generator(
                ...
                steps_per_epoch=2 if params_dict['quick_test'] else data_gen_train.get_total_batches_in_data(),
                epochs=params_dict['epochs_per_fit'],
                verbose=2,
            )
            ...
            pred = model.predict_generator(
                ...
                steps=2 if params_dict['quick_test'] else data_gen_val.get_total_batches_in_data(),
                verbose=2
            )

            sed_pred = evaluation_metrics.reshape_3Dto2D(pred[0]) > 0.5
            doa_pred = evaluation_metrics.reshape_3Dto2D(pred[1] if params_dict['doa_objective'] is 'mse'
                                                         else pred[1][:, :, nb_classes:])
            ...
            cls_new_metric = SELD_evaluation_metrics.SELDMetrics(nb_classes=data_gen_val.get_nb_classes(),
                                                                 doa_threshold=params_dict['lad_doa_thresh'])
            ...
            path = '{}.png'.format(unique_name)
            showImage.choosepic(path=path, widget=widget)
            ...
            if patience_cnt > params_dict['patience']:
                break
        ...
        if 'eval' in data_format:
            data.append([data_format, '2020'])
            data.append([params_dict['model_name'], round(new_metric[best_epoch, 2],2),
                         round(new_metric[best_epoch, 3] * 100,2),round(new_metric[best_epoch, 0],2),
                         round(new_metric[best_epoch, 1] * 100,2)])
            data.append([data_format, '2019'])
            data.append([params_dict['model_name'], round(doa_metric[best_epoch, 0],2),
                         round(doa_metric[best_epoch, 1]*100,2),
                         round(sed_metric[best_epoch, 0],2), round(sed_metric[best_epoch, 1]*100,2)])
            save_result(data=data, params_dict=params_dict)

        # ------------------  Calculate metric scores for unseen test split ---------------------------------
        print('\nLoading the best model and predicting results on the testing split')
        print('\tLoading testing dataset:')
        data_gen_test = cls_data_generator.DataGenerator(
            params_dict=params_dict, split=split, shuffle=False, per_file=params_dict['dcase_output'],
            is_eval=True if params_dict['mode'] is 'eval' else False
        )

        model = keras_model.load_seld_model('{}_model.h5'.format(unique_name), params_dict['doa_objective'])
        pred_test = model.predict_generator(
            generator=data_gen_test.generate(),
            steps=2 if params_dict['quick_test'] else data_gen_test.get_total_batches_in_data(),
            verbose=2
        )

        test_sed_pred = evaluation_metrics.reshape_3Dto2D(pred_test[0]) > 0.5
        test_doa_pred = evaluation_metrics.reshape_3Dto2D(pred_test[1] if params_dict['doa_objective'] == 'mse'
                                                          else pred_test[1][:, :, nb_classes:])

        # 是否输出结果
        if params_dict['dcase_output']:
            # Dump results in DCASE output format for calculating final scores
            dcase_dump_folder = os.path.join(params_dict['dcase_dir'],
                                             '{}_{}_{}'.format(params_dict['model_name'],
                                                               params_dict['dataset'], params_dict['mode']))
        ...
        if params_dict['mode'] == 'dev':
            ...
            test_gt = collect_test_labels(data_gen_test, test_data_out, nb_classes, params_dict['quick_test'])
            ...
            avg_scores_test.append([test_new_metric[0], test_new_metric[1], test_new_metric[2],
                                    test_new_metric[3], test_new_seld_metric])
            print('Results on test split:')

            print('\tDCASE2020 Scores')
            print('\tClass-aware localization scores: DOA Error: {:0.1f}, F-score: {:0.1f}'
            ...
            print('\n\tDCASE2019 Scores')
            print('\tLocalization-only scores: DOA Error: {:0.1f}, Frame recall: {:0.1f}'
            ...
            data = []
            data.append([data_format, '2020'])
            data.append([params_dict['model_name'], round(test_new_metric[2],2), round(test_new_metric[3]*100,2),
                         round(test_new_metric[0],2), round(test_new_metric[1]*100,2)])
            data.append([data_format, '2019'])
            data.append([params_dict['model_name'], round(test_doa_loss[0],2), round(test_doa_loss[1]*100,2),
                         round(test_sed_loss[0],2), round(test_sed_loss[1]*100,2)])
            save_result(data=data, params_dict=params_dict)
    return model_name

'''预测'''
def predict(params_dict, path):
    ...
    #     if i == 0:
    #         result_path = string
    #     result_path = result_path + '_' + string
    #     if string == 'dev' or string == 'eval':
    #         break

    if '_dev_' in path:
        params_dict['mode'] = 'dev'
    elif '_eval_' in path:
        params_dict['mode'] = 'eval'
    else:
        print("无法识别模型")

    if '_mic_' in path:
        params_dict['dataset'] = 'mic'
    elif '_foa_' in path:
        params_dict['dataset'] = 'foa'
    else:
        print("无法识别模型")
    ...
    for split_cnt, split in enumerate(test_splits):
        ...
        model = keras_model.load_seld_model(path, params_dict['doa_objective'])
        model.summary()
        pred_test = model.predict_generator(
            generator=data_gen_test.generate(),
            steps=2 if params_dict['quick_test'] else data_gen_test.get_total_batches_in_data(),
        ...
        if params_dict['dcase_output']:
            # Dump results in DCASE output format for calculating final scores
            dcase_dump_folder = os.path.join(params_dict['dcase_dir'],
                                             '{}_{}_{}'.format(params_dict['model_name'], params_dict['dataset'],
                                                               params_dict['mode']))
            cls_feature_class.create_folder(dcase_dump_folder)
            print('Dumping recording-wise results in: {}'.format(dcase_dump_folder))
...
#         params.set_model_name('1_4')
#         params_dict = params.get_params()
#         sys.exit(predict(params_dict, 'D:\MyProgram\Python\SE4SELD\models\\1_4_mic_dev_split1_model.h5'))
